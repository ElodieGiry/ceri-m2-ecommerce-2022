...
def connexion(email, password):
    ...
    if len(result) == 1:
        query=f'SELECT prenomClient, nomClient, telephoneClient, FROM `client` WHERE emailClient = \'{email}\' AND mdpClient = \'{password}\''
        cursorDatabase.execute(query)
        result=cursorDatabase.fetchall()
        return('Bonjour ' + result[0][0]+ ' ' + result[0][1], result[0][2])
    else:
